...
const createSendToken = (user, statusCode, res) => {
    ...
    if(process.env.NODE_ENV === 'production') coookieOptions.secure = false; 
...
exports.login =  catchAsync(async (req, res, next) => {
    //console.log('Request hit login controller')
...
exports.protect = catchAsync(async(req, res, next) => {
    ...
    if(!token) {
        return next(new AppError
            ('You are not logged in, please login..!', 401));
    }
    ...
    if(currentUser.changedPasswordAfter(decoded.iat)){
        return next(new AppError('User recently changed password, login required', 401));
    }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    // If we try to save before removing validation we get an error 
    // as we marked password and email as mandate in our user model
